...
root_domain = 'https://dc.healthinspections.us'
...
def scrape_restaurants():
    ...
    url = '%s/webadmin/dhd_431/web/index.cfm' % root_domain
    ...
    for i, restaurant in enumerate(restaurant_list):
        ...
        try:
            ...
            if restaurant.select('h3 a')[0].text.strip() != '':
                ...
                # restaurant_dict['inspection_url'] = 'https://washington.dc.gegov.com/webadmin/dhd_431/web/index.cfm?a=inspections&permitID=%s' % restaurant_dict['permit_id']
                restaurant_dict['inspection_url'] = '%s/webadmin/dhd_431/web/index.cfm?a=inspections&permitID=%s' % (
                    root_domain, restaurant_dict['permit_id'])
                    ...
                    if string != '':
                        ...
                        if index == 2:
                            ...
                            try:
                                ...
                                restaurant_dict['ward'] = (string.split('Ward: Ward')[1].split('|')[0].strip())
                                # print(restaurant_dict['ward'])
    ...
    for i, restaurant in enumerate(restaurants):
        ...
        for link in soup.select('#divInspectionSearchResultsListing a'):
            ...
            try:
                ...
                inspection_dict['inspection_url'] = "%s/webadmin/dhd_431/lib/mod/inspection/paper/_paper_food_inspection_report.cfm?wguid=1367&wgunm=sysact&wgdmn=431&inspectionID=%s" % (
                    root_domain, inspection_dict['inspection_id'])
...
def parse_inspection_html(inspection_list=None):
    ...
    for i, inspection in enumerate(inspections):
        ...
        if len(soup.select('h3')) > 0 and 'Page Temporarily Unavailable' in soup.select('h3')[0].text.strip():
            # Somes scraped files are just a page temporarily unavailable.
            # Try rescraping
            print('Initially scraped page unavailable.\nAttempting to rescrape...')
            r = requests.get(inspection['inspection_url'])
            soup = BeautifulSoup(r.content)
        ...
        try:
            ...
            inspection_dict['type'] = soup.select('div.container > div')[6].text.strip()
        except IndexError:
            # Somes scraped files are just a page temporarily unavailable.
            # Skip for now.
            print('html not well formed. Skipping...')
            continue
        ...
        time_part = soup.select('html > body > table')[1].select('table')[0].select('tr')[4].text.split('\n')
        ...
        try:
            crit = int(soup.select('html > body > table')[1].select('table')[1].select('tr')[0].select('td')[1].text.strip())
        except ValueError:
            crit = 0

        try:
            cos = int(soup.select('html > body > table')[1].select('table')[1].select('tr')[0].select('td')[3].text.strip())
        except ValueError:
            cos = 0

        try:
            ncrit = int(soup.select('html > body > table')[1].select('table')[1].select('tr')[1].select('td')[1].text.strip())
        except ValueError:
            ncrit = 0

        try:
            ncos = int(soup.select('html > body > table')[1].select('table')[1].select('tr')[1].select('td')[3].text.strip())
        except ValueError:
            ncos = 0
    ...
    print('Parsed %s total inspections' % len(updated_inspections))
...
def scrape_latest_inspections():
    ...
    latest_inspection = Inspection.objects.all().order_by('-time_in')[0]
    print(latest_inspection)
